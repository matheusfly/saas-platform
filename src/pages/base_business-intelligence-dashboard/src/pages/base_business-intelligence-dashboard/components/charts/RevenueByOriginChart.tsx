...
import { RevenueByOriginData } from '../../types';
...
const RevenueByOriginChart: React.FC<Props> = ({ data }) => {
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} layout="vertical" margin={{ top: 5, right: 80, left: 20, bottom: 20 }}>
        ...
        <XAxis type="number" stroke="#858360" tickFormatter={currencyFormatter}>
            <Label value="Receita Total (R$)" offset={-10} position="insideBottom" fill="#858360" />
        </XAxis>
        <YAxis type="category" dataKey="origin" stroke="#858360" width={100} />
        <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(176, 210, 54, 0.2)' }}/>
        <Bar dataKey="revenue" fill="#858360">
            <LabelList dataKey="revenue" position="right" style={{ fill: '#1E1E1E' }} formatter={(value: number) => `R$${value.toLocaleString('pt-BR')}`} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
export default RevenueByOriginChart;
