...
import { FilterState, RevenueExpenseData } from '../types';
...
const generateForecastData = (historicalData: RevenueExpenseData[]) => {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    // Simple growth model: revenue grows by a small random percentage
    lastRevenue *= (1 + (Math.random() * 0.05 + 0.01)); // 1% to 6% growth
    // Expenses grow slightly slower than revenue for a positive net trend
  ...
  const combinedData = [...historicalData, ...forecastOnly];
