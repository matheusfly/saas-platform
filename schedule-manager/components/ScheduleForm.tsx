...
import { TrashIcon, PlusIcon, InformationCircleIcon, ClockIcon, CalendarIcon } from './icons';
...
const ClassForm: React.FC<ClassFormProps> = ({ 
  ...
  const iconClasses = "h-5 w-5 text-gray-400 dark:text-sage absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none";
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
            <div className="relative">
                ...
                <select name="classType" value={formData.classType} onChange={handleChange} className={`${baseInputClasses} pl-10`}>
                    {Object.values(ClassType).filter(t => t !== ClassType.PONTO).map(type => <option key={type} value={type}>{type}</option>)}
                </select>
            </div>
            <div className="relative">
                <InformationCircleIcon className={iconClasses} />
                <input type="number" name="capacity" value={formData.capacity} onChange={handleChange} className={`${baseInputClasses} pl-10 ${errors.capacity ? errorBorderClasses : ''}`} placeholder="Vagas" />
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
            <div className="relative">
                <InformationCircleIcon className={iconClasses} />
                <select name="isRecurring" value={String(formData.isRecurring)} onChange={handleChange} className={`${baseInputClasses} pl-10`}>
             ...
             <div className="relative">
                <InformationCircleIcon className={iconClasses} />
                <select name="repeatType" value={formData.repeatType} disabled={!formData.isRecurring} onChange={handleChange} className={`${baseInputClasses} pl-10 disabled:opacity-50`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {formData.isRecurring ? (
              ...
              <div>
                 ...
                 <button type="button" onClick={(e) => handleOpenPopover('date', e)} className="relative w-full text-left">
                     <CalendarIcon className={iconClasses} />
                     <span className={`${baseInputClasses} block pl-10`}>{formData.startDate.toLocaleDateString('pt-BR')}</span>
                 </button>
              </div>
          )}
          ...
          <div>
             ...
             <button type="button" onClick={(e) => handleOpenPopover('time', e)} className={`relative w-full text-left ${errors.startTime ? errorBorderClasses : ''}`}>
                 <ClockIcon className={iconClasses} />
                 <span className={`${baseInputClasses} block pl-10`}>{formData.startTime.toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'})}</span>
             </button>
          </div>
        </div>
