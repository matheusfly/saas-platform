...
export const useSchedule = () => {
  ...
  const checkIn = useCallback(async (teacherId: string, checkInTime: Date) => {
    const { newLog, newEntry } = await api.checkIn(teacherId, checkInTime);
    setWorkLogs(prev => [...prev, newLog]);
    if (newEntry) {
        setSchedule(prev => [...prev, newEntry]);
    } else {
        // an existing entry was linked, we need to re-render it
        const linkedEntry = schedule.find(e => e.teacherIds.includes(teacherId) && checkInTime >= e.startTime && checkInTime <= e.endTime);
        if(linkedEntry) {
            setSchedule(prev => prev.map(e => e.id === linkedEntry.id ? {...e, workLogId: newLog.id} : e));
        }
    }
  }, [schedule]);
