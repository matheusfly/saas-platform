...
const Dashboard: React.FC<DashboardProps> = ({ filters, onFilterChange, chartData }) => {
  // Extract metrics directly from chartData as they are top-level properties in AllData
  const {
    metrics: {
      averageLtv,
      newClientCount,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <ChartCard title="Funil de Conversão" chartClassName="h-[450px]">
            <ConversionFunnelChart data={chartData.funnelDropOff} />
          </ChartCard>
        </div>
        <div className="lg:row-span-2">
          <ChartCard title="Evolução de Leads" chartClassName="h-full">
            <LeadEvolutionChart data={[]} />
          </ChartCard>
        </div>
      </div>
